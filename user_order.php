...
if ($result->num_rows > 0) {
    ...
    foreach ($orders as $order_id => $order_items) {
        ...
        echo '<a href="user_order_status.php?orders_status_id=' . $order_id . '" class="order-link">';
        ...
        echo '</a>';
        echo '<p class="order-status ' . $status_class . '" id="status-text-' . $order_id . '" data-order-id="' . $order_id . '">';
        echo $status_text;
        echo '</p>';
...
?>
    ...
    function fetchOrderStatus(orderId) {
        fetch(`get_order_status_user.php?orders_status_id=${orderId}`)
            .then(response => response.json())
            .then(data => {
                const statusTextElement = document.querySelector(`#status-text-${orderId}`);
                const statusOrder = data.status_order;
                let statusClass = '';
                let statusText = '';

                if (statusOrder === 'receive') {
                    statusText = 'รับออเดอร์';
                ...
                if (statusTextElement) {
                    statusTextElement.className = `order-status ${statusClass}`;
